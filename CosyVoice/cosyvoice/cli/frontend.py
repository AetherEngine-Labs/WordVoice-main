...
class CosyVoiceFrontEnd:
    ...
    def frontend_zero_shot_wordvoice(self, tts_text, prompt_text, prompt_wav, word_list, start_list, dur_list, bnd_list, tone_list, eng_list, f0_list, resample_rate, zero_shot_spk_id):
        tts_text_token, tts_text_token_len = self._extract_text_token(tts_text)
        word_list = [self._extract_text_token(word)[0] for word in word_list]
        ...
        if zero_shot_spk_id == '':
            ...
            model_input = {'prompt_text': prompt_text_token, 'prompt_text_len': prompt_text_token_len,
                           ...
                           'prompt_speech_feat': speech_feat, 'prompt_speech_feat_len': speech_feat_len,
                           'llm_embedding': embedding, 'flow_embedding': embedding,
                           'word_list': word_list, 'start_list': start_list, 'dur_list': dur_list,
                           'bnd_list': bnd_list, 'tone_list': tone_list, 'eng_list': eng_list, 'f0_list': f0_list}
        else:
            model_input = {**self.spk2info[zero_shot_spk_id]}
        model_input['text'] = tts_text_token
        model_input['text_len'] = tts_text_token_len
