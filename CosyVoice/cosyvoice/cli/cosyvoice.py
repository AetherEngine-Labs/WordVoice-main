...
class WordVoice(CosyVoice2):
    ...
    def wordvoice_inference(
        self, tts_text, prompt_text, prompt_wav, word_list, start_list, dur_list,
        bnd_list, tone_list, eng_list, f0_list,
        ...
        for i in tqdm(self.frontend.text_normalize(tts_text, split=False, text_frontend=text_frontend)):
            if (not isinstance(i, Generator)) and len(i) < 0.5 * len(prompt_text):
                logging.warning('synthesis text {} too short than prompt text {}, this may lead to bad performance'.format(i, prompt_text))
            model_input = self.frontend.frontend_zero_shot_wordvoice(i, prompt_text, prompt_wav, word_list, start_list, dur_list, 
                                                                     bnd_list, tone_list, eng_list, f0_list, self.sample_rate, zero_shot_spk_id)
            start_time = time.time()
            logging.info('synthesis text {}'.format(i))
            for model_output in self.model.wordvoice_tts(**model_input, stream=stream, speed=speed):
                speech_len = model_output['tts_speech'].shape[1] / self.sample_rate
                logging.info('yield speech len {}, rtf {}'.format(speech_len, (time.time() - start_time) / speech_len))
                yield model_output
                start_time = time.time()
